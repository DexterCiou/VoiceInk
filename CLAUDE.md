...
└── VoiceInk/
    ...
    │
    └── Resources/
        └── DefaultPrompt.txt          # 預設潤飾規則（英文指令框架 + 10 條潤飾規則，永遠生效）
```
...
3. **Polishing rules（潤飾規則，共 10 條）**：
   ...
   8. 移除重複語句（保留最完整的一句）
   9. **段落格式化**：當輸出超過 4 句時，每 2-3 句在句號處插入空行（雙換行），短文（1-3 句）維持單段
   10. 字典詞彙優先替換（發音相近時必須替換，最高優先級）
...
- 長文需自動分段（每 2-3 句換行），否則整段文字貼上後不易閱讀
...
| **設定頁 API Key 不顯示**（已修復） | SettingsViewModel 在 `init()` 時載入 API Key，但 onboarding 完成後才儲存 Key，時序不對 | SettingsView 加入 `.onAppear { settingsViewModel.loadAPIKeys() }` 重新載入 |
| **關於頁面資訊精簡**（已完成） | 「語音辨識」和「文字潤飾」欄位為內部技術細節，對使用者無意義 | 移除這兩個 infoRow，僅保留系統需求和開發框架 |
...
- **目前最新版本**：`VoiceInk-1.1.6.dmg`
- **版本號規則**：每次打包遞增（如 1.1.6 → 1.1.7），舊版 .dmg 自動刪除
...
- [x] **設定頁 API Key 不顯示修復**：`.onAppear` 重新載入
- [x] **關於頁面精簡**：移除技術細節欄位
- [x] **段落格式化規則**：長文自動分段（每 2-3 句換行）
- [x] **贅詞移除擴充**：新增冗餘輔助詞（這是、那是等）
